...
describe("examples chat flow", () => {
  it("uses a page file and single-file server actions", async () => {
    ...
    expect(loginPageSource).toContain('write login: "/login" (email, password)');
    expect(loginPageSource).toContain("<!-- mdsn:block auth-nav -->");
    expect(loginPageSource).toContain('redirect "/register"');
    expect(registerPageSource).toContain("<!-- mdsn:block auth -->");
    expect(registerPageSource).toContain('write register: "/register" (username, email, password)');
    expect(registerPageSource).toContain("<!-- mdsn:block auth-nav -->");
    expect(registerPageSource).toContain('redirect "/"');
    expect(chatPageSource).toContain("<!-- mdsn:block chat -->");
    expect(chatPageSource).toContain("<!-- mdsn:block session -->");
    expect(chatPageSource).toContain('write send: "/send" (message)');
    expect(chatPageSource).toContain('read load_more: "/load-more"');
    expect(chatPageSource).toContain('write logout: "/logout"');
  ...
  it("redirects from login to the chat page and continues the chat flow", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(loginPageMarkdown).toContain('write login: "/login" (email, password)');
      expect(loginPageMarkdown).toContain('redirect "/register"');
      ...
      expect(registerPageMarkdown).toContain('write register: "/register" (username, email, password)');
      expect(registerPageMarkdown).toContain('redirect "/"');
      ...
      expect(registerResponseFragment).toContain('redirect "/chat"');
      ...
      expect(loginResponseFragment).toContain('redirect "/chat"');
      ...
      expect(chatPageMarkdown).toContain('read refresh: "/list"');
      expect(chatPageMarkdown).toContain('read load_more: "/load-more"');
      expect(chatPageMarkdown).toContain('write send: "/send" (message)');
      expect(chatPageMarkdown).toContain('write logout: "/logout"');
      ...
      expect(sessionFragment).toContain('redirect "/chat"');
      ...
      expect(refreshFragment).toContain("This view shows the most recent 50 messages.");
      expect(refreshFragment).toContain("Use `load_more` to read older messages.");
      ...
      expect(logoutResponseFragment).toContain('redirect "/"');
  ...
  it("accepts markdown action payloads for register and send", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(registerFragment).toContain('redirect "/chat"');
  ...
  it("returns markdown-first guidance for auth actions when Accept is markdown-only", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(logoutFragment).toContain('redirect "/"');
      ...
      expect(unauthorizedFragment).toContain('write login: "/login" (email, password)');
  ...
  it("requires login before reading list and load-more", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(listMarkdownBody).toContain('write login: "/login" (email, password)');
      ...
      expect(historyMarkdownBody).toContain('write login: "/login" (email, password)');
      ...
      expect(listJsonBody).toContain('write login: "/login" (email, password)');
    });
  });
  ...
  it("supports multi-agent long conversation and final summary retrieval", async () => {
    await withServer(async (baseUrl) => {
      ...
      await send(alphaCookie, "第四轮：确认 redirect 已闭环");
      ...
      expect(refreshFragment).toContain('write send: "/send" (message)');
      expect(refreshFragment).not.toContain('input agent!: text');
    });
  });
  ...
  it("lets a fresh agent complete the markdown-first end-to-end loop", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(loginSource).toContain('write login: "/login" (email, password)');
      expect(loginSource).toContain('redirect "/register"');
      ...
      expect(registerSource).toContain('write register: "/register" (username, email, password)');
      expect(registerSource).toContain('redirect "/"');
      ...
      expect(chatSource).toContain('write logout: "/logout"');
      expect(chatSource).toContain('write send: "/send" (message)');
      expect(chatSource).toContain('read refresh: "/list"');
  ...
  it("lets the client and the agent explicitly load older history", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(recentFragment).toContain("This view shows the most recent 50 messages.");
      expect(recentFragment).toContain("Use `load_more` to read older messages.");
      ...
      expect(historyFragment).toContain('read load_more: "/load-more"');
    });
  });
  ...
  it("returns a chat fragment with explicit guidance when send fails", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(failureFragment).toContain("This view shows the most recent 50 messages.");
      expect(failureFragment).toContain('write send: "/send" (message)');
