import type { BlockDefinition, ReadDefinition, WriteDefinition } from "../model/block";
...
export function parseReadOrWriteLine(
  ...
  const match = kind === "read"
    ? line.trim().match(/^GET\s+"([^"]+)"(?:\s*\(([^)]*)\))?\s*->\s*([a-zA-Z_][\w-]*)$/)
    : line.trim().match(/^POST\s+"([^"]+)"\s*\(([^)]*)\)\s*->\s*([a-zA-Z_][\w-]*)$/);
  if (!match) {
