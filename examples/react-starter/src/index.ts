...
export interface CreateReactStarterServerOptions {
  source: string;
  initialMessages?: string[];
}

export function createReactStarterServer(options: CreateReactStarterServerOptions) {
  const messages = [...(options.initialMessages ?? ["Welcome to MDSN from React"])];
