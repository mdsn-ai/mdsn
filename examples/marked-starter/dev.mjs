...
<script type="module">
  import { mountMarkedStarter } from "${withVersion("/marked-starter/client.js")}";
  mountMarkedStarter(document, window.fetch.bind(window));
</script>`;
...
const mdsn = createMarkedStarterServer({ source });
const server = http.createServer(
  createNodeHost(mdsn, {
    rootRedirect: "/guestbook",
    transformHtml: injectEnhancement,
    staticFiles: {
      "/marked-starter/index.js": join(exampleRoot, "dist", "index.js"),
      "/marked-starter/client.js": join(exampleRoot, "dist", "client.js")
    },
    staticMounts: [
      {
        urlPrefix: "/sdk/",
...
server.listen(port, "127.0.0.1", () => {
  console.log(`Marked starter demo running at http://127.0.0.1:${port}/guestbook`);
});
