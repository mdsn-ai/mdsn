...
<script type="module">
  import { mountVueStarter } from "${withVersion("/vue-starter/client.js")}";
  const host = document.getElementById("vue-starter-root");
  if (host) {
    mountVueStarter(host);
  }
</script>`;
...
const mdsn = createVueStarterServer({ source });
const server = http.createServer(
  createNodeHost(mdsn, {
    rootRedirect: "/guestbook",
    transformHtml: injectEnhancement,
    staticFiles: {
      "/vue-starter/index.js": join(exampleRoot, "dist", "index.js"),
      "/vue-starter/client.js": join(exampleRoot, "dist", "client.js")
    },
    staticMounts: [
      {
        urlPrefix: "/sdk/",
...
server.listen(port, "127.0.0.1", () => {
  console.log(`Vue starter demo running at http://127.0.0.1:${port}/guestbook`);
});
