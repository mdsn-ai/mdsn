...
```ts
import { parsePage } from "@mdsn/core";
...
```ts
import { parsePage, validatePage } from "@mdsn/core";
...
```ts
import { parseMarkdownBody } from "@mdsn/core";
...
推荐把 `@mdsn/server` 理解成“运行时 + Node host”。
...
```ts
import { createMdsnServer } from "@mdsn/server";
...
```ts
import { stream } from "@mdsn/server";
...
推荐把 `@mdsn/elements` 理解成“默认 UI 渲染器 + 自定义元素注册器”。
...
```ts
import { registerMdsnElements } from "@mdsn/elements";
