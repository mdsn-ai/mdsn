...
`@mdsn/server` is the main entry point for people who want to host MDSN on the server.
...
当 browser 走 HTML host 链时，`@mdsn/server` 会负责把 Markdown 渲染成 HTML。这个渲染能力现在支持注入：
...
如果你同时使用默认 `@mdsn/elements` UI，建议把同一个 `markdownRenderer` 对象同时传给 `mountMdsnElements(...)`，这样 server 和默认 UI 的 Markdown 呈现会保持一致。
